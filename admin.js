...
async function loadVisits() {
  ...
  const pagesBody = document.getElementById('visitsPagesTableBody');
  const pagesEmpty = document.getElementById('visitsPagesEmpty');
  if (topPages.length) {
    pagesEmpty.hidden = true;
    pagesBody.innerHTML = topPages.map(([path, count]) => `<tr><td>${path}</td><td>${count}</td></tr>`).join('');
  } else {
