...
export async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    // The client here is talking to the manager bot, not the manager
    // account itself -- once a handoff fires, point them at the direct
    // manager chat (bot-core.ts already DMs the owner with the summary,
    // using this same TELEGRAM_BOT_TOKEN, so the manager has context
    // before the client even sends that first message there).
    const reply = result.handoffUrl ? `${result.reply}\n\n${result.handoffUrl}` : result.reply;
